...
class Admin extends BaseController
{
	...
	function chkLogin($onlyAdmin = true)
	{
		if (!($this->session->userdata('logged_in'))) {
			...
			if ($onlyAdmin && $usertype != 'super_admin' && $usertype != 'admin') {
	...
	{
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['signin'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['errmsg'] = $errmsg;
	...
	{
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['register'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['errmsg'] = $errmsg;
		$data['branches'] = $this->admin_model->getAllBranches();
	...
	function changepassword()
	{
		$this->chkLogin(false);
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['changepassword'] . " - " . $data['options']['dhp_name'] . " | Dormify");
	...
	function dashboard()
	{
		$this->chkLogin(false);
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['dashboard'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		...
		$this->session->unset_userdata('dash_msg');
	...
	function tenant($tenuid, $view = 'admin')
	{
		...
		$data['title'] = ucfirst($this->lan['tenant'] . ' ' . $this->lan['profile'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'tenants';
		$data['burl'] = $this->burl;
		$data['tenuid'] = $tenuid;
		$data['view'] = $view;
		...
		$data['notify_type'] = 'success';
		$this->session->unset_userdata('tenant_flash');
	...
	function tenants($target = '', $tenuid = '')
	{
		...
		if (!empty($target)) {
			...
			$data['title'] = ucfirst($this->lan['tenants'] . " - " . $data['options']['dhp_name'] . " | Dormify");
			$data['cpage'] = 'tenants';

			$ten_list_msg = $this->session->flashdata('ten_list_msg');
			$data['notify_msg'] = isset($ten_list_msg) ? $ten_list_msg : '';
			$data['notify_type'] = 'success';
			$this->session->unset_userdata('ten_list_msg');
	...
	function newtenant()
	{
		...
		$data['branches'] = $this->admin_model->getAllBranches();
		$data['title'] = ucfirst($this->lan['new'] . ' ' . $this->lan['tenant'] . " - " . $data['options']['dhp_name'] . " | Dormify");
	...
	function edittenant($tenuid)
	{
		...
		$data['userdata'] = $this->session->userdata;
		$data['tendata'] = $this->admin_model->getTenantDetails($tenuid);
		$data['title'] = ucfirst($this->lan['edit'] . ' ' . $this->lan['tenant'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'newtenant';
		$data['burl'] = $this->burl;
		$data['tenuid'] = $tenuid;
		$this->load->view('backend/common/header', $data);
	...
	function assignTenantBed($tenuid)
	{
		...
		$data['title'] = ucfirst($this->lan['assign'] . ' ' . $this->lan['bed'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'newtenant';
		$data['tenuid'] = $tenuid;
		$this->load->view('backend/common/header', $data);
	...
	function newinvoice()
	{
		...
		$data['notify_type'] = 'success';
		$this->session->unset_userdata('invoice_list_msg');
		$data['title'] = ucfirst($this->lan['generate'] . ' ' . $this->lan['invoice'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'invoices';
		$this->load->view('backend/common/header', $data);
	...
	function savetenant()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($tenuid == 0) {
				...
				if (!empty($options['smtp_host']) && !empty($options['smtp_username']) && !empty($options['smtp_pass'])) {
					$content = getEmailTemplate('email_tenant_update');
					$maildata = array(
						'{{ten_name}}' => html_escape($this->input->post('ten_name')),
						'{{dhpm_name}}' => $options['dhp_name'],
					);
	...
	function rooms($target = '', $roomuid = '')
	{
		...
		if (!empty($target)) {
			...
			$data['title'] = ucfirst($this->lan['rooms'] . " - " . $data['options']['dhp_name'] . " | Dormify");
			$data['cpage'] = 'rooms';

			$room_list_msg = $this->session->flashdata('room_list_msg');
			$data['notify_msg'] = isset($room_list_msg) ? $room_list_msg : '';
			$data['notify_type'] = 'success';
			$this->session->unset_userdata('room_list_msg');
	...
	function newroom()
	{
		$this->chkLogin();
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['new'] . ' ' . $this->lan['room'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'newroom';
		$this->load->view('backend/common/header', $data);
	...
	function editroom($roomuid)
	{
		...
		$data['title'] = ucfirst($this->lan['edit'] . ' ' . $this->lan['room'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'editroom';
	...
	function roomstructure($roomuid)
	{
		...
		$data['title'] = ucfirst($this->lan['room'] . ' ' . $this->lan['structure'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'rooms';
		$data['roomuid'] = $roomuid;
		$this->load->view('backend/common/header', $data);
	...
	function beds($target = '', $beduid = '', $tenuid = '', $reqid = '')
	{
		...
		if (!empty($target)) {
			...
			$data['options'] = $this->gmsOptions();
			$data['title'] = ucfirst($this->lan['beds'] . " - " . $data['options']['dhp_name'] . " System");
			...
			$data['notify_type'] = 'success';
			$this->session->unset_userdata('bed_list_msg');
	...
	function newbed()
	{
		...
		$data['options'] = $this->gmsOptions();
		$data['allrooms'] = $this->admin_model->getAllRooms($this->branchid);
		$data['title'] = ucfirst($this->lan['new'] . ' ' . $this->lan['bed'] . " - " . $data['options']['dhp_name'] . " System");
		$data['cpage'] = 'newbed';
		$this->load->view('backend/common/header', $data);
	...
	function editbed($beduid)
	{
		...
		$data['title'] = ucfirst($this->lan['edit'] . ' ' . $this->lan['bed'] . " - " . $data['options']['dhp_name'] . " System");
		$data['cpage'] = 'newbed';
		$data['beduid'] = $beduid;
		$this->load->view('backend/common/header', $data);
		$this->load->view('backend/newbed', $data);
	...
	function assignbed($beduid, $tenuid = '', $reqid = '')
	{
		...
		$data['allrooms'] = $this->admin_model->getAllRooms($this->branchid);
		$data['beddata'] = $this->admin_model->getBedDetails($beduid);
		$data['title'] = ucfirst($this->lan['assign'] . ' ' . $this->lan['bed'] . " - " . $data['options']['dhp_name'] . " System");
		$data['cpage'] = 'newbed';
		$data['beduid'] = $beduid;
		$data['tenuid'] = $tenuid;
		$data['reqid'] = $reqid;
	...
	function requestbed()
	{
		...
		$data['allrooms'] = $this->admin_model->getAllRooms($this->branchid);
		$data['allbeds'] = $this->admin_model->getAllBeds();
		$data['title'] = ucfirst($this->lan['requestbed'] . " - " . $data['options']['dhp_name'] . " System");
	...
	function requestes()
	{
		...
		$data['title'] = ucfirst($this->lan['requestes'] . " - " . $data['options']['dhp_name'] . " System");
		$data['tenuid'] = $this->session->userdata['user_id'];
		$data['cpage'] = 'makerequest';
		$this->load->view('backend/common/header', $data);
	...
	function allrequests()
	{
		$this->chkLogin(false);
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['allrequests'] . " - " . $data['options']['dhp_name'] . " System");
		$data['tenuid'] = $this->session->userdata['user_id'];
		$usertype = $this->session->userdata['user_type'];
		...
		$data['notify_type'] = 'success';
		$this->session->unset_userdata('req_list_msg');
	...
	function rejectrequest($reqid)
	{
		...
		if (!empty($options['smtp_host']) && !empty($options['smtp_username']) && !empty($options['smtp_pass'])) {
			$req = $this->admin_model->getRequests($reqid);
			$ten = $req['tenant_details'];
			$content = getEmailTemplate('reject_request');
			$maildata = [
	...
	{
		$this->chkLogin();
		$this->admin_model->acceptReq($reqid);
		$msg = $this->lan['acceptedmsg'];
		$this->session->set_flashdata('req_list_msg', $msg);
		redirect('admin/allrequests', 'refresh');
	}
	...
	function processbed()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$assbed = $this->admin_model->assignBed($beduid);
	...
	function processTenBed()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->admin_model->assignBed($tenuid);
			$msg = $this->lan['bed'] . ' ' . $this->lan['successfully'] . ' ' . $this->lan['assigned'];
			$this->session->set_flashdata('ten_list_msg', $msg);
			redirect('admin/tenants', 'refresh');
		}
	...
	function processes()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$assignEs = $this->admin_model->assignEs($esid);
	...
	function extraservices($target = '', $extraservicesuid = '', $tenuid = '', $reqid = '')
	{
		...
		if (!empty($target)) {
			...
			$data['title'] = ucfirst($this->lan['extraservices'] . " - " . $data['options']['dhp_name'] . " | Dormify");
			...
			$data['notify_type'] = 'success';
			$this->session->unset_userdata('extraservices_list_msg');
	...
	function assignextraservices($esid, $tenuid, $reqid)
	{
		...
		$data['title'] = ucfirst($this->lan['assign'] . ' ' . $this->lan['extraservice'] . " - " . $data['options']['dhp_name'] . " | Dormify");
	...
	function newextraservice()
	{
		$this->chkLogin();
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['new'] . ' ' . $this->lan['extraservice'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'newextraservice';
		$this->load->view('backend/common/header', $data);
	...
	function editextraservice($esid)
	{
		...
		$data['allrooms'] = $this->admin_model->getAllRooms();
		$data['esdata'] = $this->admin_model->getEsDetails($esid);
		$data['title'] = ucfirst($this->lan['edit'] . ' ' . $this->lan['extraservice'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'newextraservice';
	...
	function invoices()
	{
		...
		$data['title'] = ucfirst($this->lan['invoices'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'invoices';
	...
	function invoice($inv_id)
	{
		...
		$data['title'] = ucfirst($this->lan['invoice'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'invoices';
	...
	function users()
	{
		...
		$gdata['title'] = ucfirst($this->lan['users'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'users';
		...
		$crud->change_field_type('user_type', 'enum', array('super_admin', 'admin'));
	...
	function branches()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['branches'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
	...
	function expensecat()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['expense'] . ' ' . $this->lan['categories'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'settings';
	...
	function ubillcat()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst("utility bill categories - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'settings';
	...
	function complaintcat()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['complaint'] . ' ' . $this->lan['categories'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'settings';
	...
	function idcat()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst("ID Document categories - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'settings';
	...
	function emailtpl()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['email'] . ' ' . $this->lan['templates'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'settings';
	...
	function webpages()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['web'] . ' ' . $this->lan['pages'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'website';
	...
	function blog()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['blog'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'website';
	...
	function gallery()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['gallery'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'website';
	...
	function events()
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['events'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'events';
		...
		if ($gdata['userdata']['user_type'] != 'super_admin' && $gdata['userdata']['user_type'] != 'admin') {
			$crud->unset_add();
			$crud->unset_edit();
			$crud->unset_delete();
			$crud->unset_clone();
	...
	function filemanager()
	{
		$this->chkLogin(false);
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['filemanager'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'website';
		$data['connector'] = site_url() . '/admin/connector';
		$dash_msg = $this->session->flashdata('dash_msg');
		$data['notify_msg'] = isset($dash_msg) ? $dash_msg : '';
	...
	function about()
	{
		$this->chkLogin(false);
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['aboutus'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'aboutus';
		$dash_msg = $this->session->flashdata('dash_msg');
		$data['notify_msg'] = isset($dash_msg) ? $dash_msg : '';
		$data['notify_type'] = 'success';
	...
	function notices($view = 'admin')
	{
		...
		$this->load->library('grocery_CRUD');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['notices'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
		$gdata['cpage'] = 'notices';
		$gdata['userdata'] = $this->session->userdata;
		...
		if (!empty($this->branchid)) {
			$crud->where('notice_branch', $this->branchid);
			$crud->or_where('notice_branch', '');
		}
		...
		if ($gdata['userdata']['user_type'] == 'admin') {
	...
	function payments($target = '', $paymentuid = '')
	{
		...
		if (!empty($target)) {
			...
			$data['options'] = $this->gmsOptions();
			$data['title'] = ucfirst($this->lan['payments'] . " - " . $data['options']['dhp_name'] . " | Dormify");
			$data['cpage'] = 'payments';
			...
			$data['notify_type'] = 'success';
			$this->session->unset_userdata('payment_list_msg');
	...
	function newpayment()
	{
		...
		$data['title'] = ucfirst($this->lan['new'] . ' ' . $this->lan['payment'] . " - " . $data['options']['dhp_name'] . " | Dormify");
	...
	function makepayment()
	{
		...
		$data['title'] = ucfirst($this->lan['make'] . ' ' . $this->lan['payment'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'payments';
		$payment_list_msg = $this->session->flashdata('payment_list_msg');
		$data['notify_msg'] = isset($payment_list_msg) ? $payment_list_msg : '';
		$data['notify_type'] = 'success';
	...
	function expenses($target = '', $expenseuid = '')
	{
		...
		$this->load->library('grocery_crud');
		$gdata['options'] = $this->gmsOptions();
		$gdata['title'] = ucfirst($this->lan['expenses'] . " - " . $gdata['options']['dhp_name'] . " | Dormify");
	...
	function newexpense()
	{
		$this->chkLogin();
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['new'] . ' ' . $this->lan['expense'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'newpayment';
		$this->load->view('backend/common/header', $data);
	...
	{
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['income'] . ' ' . $this->lan['report'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'report';
		...
		$data['notify_type'] = 'success';
		$this->session->unset_userdata('incomereport_list_msg');
		$data['report'] = 'income';
	...
	{
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['expense'] . ' ' . $this->lan['report'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		$data['cpage'] = 'report';
		$incomereport_list_msg = $this->session->flashdata('incomereport_list_msg');
		$data['notify_msg'] = isset($incomereport_list_msg) ? $incomereport_list_msg : '';
		$data['notify_type'] = 'success';
	...
	function configurations()
	{
		$this->chkLogin();
		$data['options'] = $this->gmsOptions();
		$data['title'] = ucfirst($this->lan['configurations'] . " - " . $data['options']['dhp_name'] . " | Dormify");
		...
		$data['notify_type'] = 'success';
		$this->session->unset_userdata('config_list_msg');
	...
	function complaints()
	{
		...
		$data['options'] = $this->gmsOptions();
		$data['userdata'] = $this->session->userdata;
		$data['title'] = ucfirst($this->lan['complaints'] . " - " . $data['options']['dhp_name'] . " | Dormify");
	...
	function processinv()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->admin_model->createInvoice();
