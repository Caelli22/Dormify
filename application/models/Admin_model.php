...
class Admin_model extends CI_Model
{
	...
	{
		$adminquery = $this->loginAdmin($email, $password);
	...
	{
		if (!empty($branch)) {
			$this->db->where('ten_branch', $branch);
		}
		$return['tenants'] = $this->db->count_all_results('tbl_tenants');

		if (!empty($branch)) {
			$this->db->where('room_branch', $branch);
		}
	...
	function getAllTenants($branch = '')
	{
		...
		foreach ($rows as $rowid => $tenant) {
			...
			if (is_array($branchdetails) && count($branchdetails) > 0) {
				$rows[$rowid]['branch'] = $branchdetails['branch_name'];
			} else {
				$rows[$rowid]['branch'] = 'N/A';
			}
		}
		return $rows;
	}
	...
	{
		if (!empty($branch)) {
			$this->db->where('es_branch', $branch);
			$this->db->or_where('es_branch', '');
		}
		...
		foreach ($rows as $esid => $es) {
			...
			if (is_array($branchdetails) && count($branchdetails) > 0) {
				$rows[$esid]['branch'] = $branchdetails['branch_name'];
			} else {
				$rows[$esid]['branch'] = 'N/A';
			}
		}
		return $rows;
	}
	...
	function getAllRequests($ten_uid = '')
	{
		...
		foreach ($requests as $rid => $request) {
			if ($request['request_type'] == 'BED') {
				...
				$this->db->where('ten_uid', $request['request_ten_uid']);
				$tenant = $this->db->get('tbl_tenants')->row_array();
				$requests[$rid]['tenant'] = $tenant;
	...
	function getRequests($reqid)
	{
		$return['request'] = $this->db->where('id', $reqid)->get("tbl_requests")->row_array();
		$return['tenant_details'] = $this->db->where('ten_uid', $return['request']['request_ten_uid'])->get("tbl_tenants")->row_array();
		return $return;
	...
	function getTenantDetails($tenuid)
	{
		...
		if (is_array($branchdetails) && count($branchdetails) > 0) {
			$rows['branch'] = $branchdetails['branch_name'];
		} else {
			$rows['branch'] = 'N/A';
		}
		return $rows;
	...
	function requestes($ten_uid)
	{
		...
		$data = array(
			...
			'request_notes' => html_escape($this->input->post('note')),
			'request_create_date' => $date,
			'request_status' => 'PENDING'
		);
		$this->db->insert('tbl_requests', $data);
